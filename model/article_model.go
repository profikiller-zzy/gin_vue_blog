...
type ArticleModel struct {
	...
	CommentModels []CommentModel `gorm:"foreignKey:ArticleID" json:"-"`           // 文章的评论列表，文章-评论列表为普通一对多
	User          UserModel      `json:"-"`                                       // 文章的作者
	UserID        uint           `json:"user_id"`                                 // 文章作者的ID
	NickName      string         `gorm:"size:36" json:"nick_name"`                // 文章作者的昵称
	Category      CategoryModel  `gorm:"foreignKey:CategoryID" json:"-"`          // 文章分类，文章-分类为多对一
